...
namespace ConsoleApplication1
{
    class Server: TcpServer
    {
                ...
                string content = string.Format(
                    "<env:Envelope xmlns:env=\"http://www.w3.org/2001/09/soap-envelope\">" +
                        "<env:Header>" +
                            ...
                            (encrypted ? "<sec:Encryption xmlns:sec=\"http://security.org/\" env:mustUnderstand=\"true\"/>" : "") +
                       "</env:Header><env:Body>" +
        ...
        private string createSOAPBody( string company, string price, bool encrypted )
        {
            ...
            if ( encrypted )
                body = string.Format( "<sec:EncryptedContent xmlns:sec=\"http://security.org/\">{0}</sec:EncryptedContent>", Encryption.Encrypt( body, "password" ) );
            return body;
        }
